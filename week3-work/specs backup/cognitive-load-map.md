...
**Finding 4 — The passive confirmation model (silence = accepted) creates a structural blind spot.** No confirmation step captures nurse intent between submission and shift start. This is the root cause of the addressable no-show segment (A9) and is an explicit design gap the Shift Confirmation Notifier (MVP Feature 5) must close.
