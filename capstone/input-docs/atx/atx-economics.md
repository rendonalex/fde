...
### Self-hosted / on-premises inference (alternative cost model — conditional, not default)

API per-token pricing is the default cost shape and the right baseline. **Self-hosted inference** — running open-source models (Llama, Mistral, Qwen, Phi, fine-tuned derivatives) on EPAM-managed or client-owned hardware — is a *conditional* alternative, valid in specific cases but typically **slower per inference** and carrying real ops overhead participants under-count. Do not default to it; consider it only when a specific force pushes you off the API baseline.

The cost structure differs fundamentally:

| Cost component | API model (default) | Self-hosted model |
|---|---|---|
| Marginal per-inference cost | High (per-token rate × volume) | Near-zero once hardware is paid for |
| Capital cost | None | GPU / inference hardware up front |
| Operating cost | None beyond API spend | Electricity + cooling + **ops staff** + amortisation |
| Inference latency | Fast (frontier-tail) | **Typically slower** than frontier APIs |
| Capability | Frontier-class out of the box | Lags frontier by 6–18 months; closes via fine-tuning |
| Floor cost | Scales with usage — zero at zero volume | Fixed regardless of utilisation |
| Failure-mode cost | Provider absorbs uptime risk | Client/EPAM absorbs uptime risk |

When self-hosted is genuinely the right call (one of these forces applies — don't pick self-hosted without one):

- **Data-sovereignty / compliance** rules out external APIs (regulated verticals, classified workloads, data-residency law).
- **Volume genuinely amortises the capital** at the right point on the curve. Single-instance H100-class hardware is ~£25–35k capital + ~£3–6k/year electricity + ops staff. Crossover vs API for a Sonnet-equivalent workload typically lands somewhere in the millions of cases/year — verify with your specific workload's token shape. **High volume alone isn't enough — the latency and ops cost still have to be acceptable.**
- **Fine-tuning needs** that produce a model the client owns and re-trains on their own data, where the fine-tune is load-bearing for accuracy (rare — most production agents do not need fine-tuning).

When self-hosted is the wrong call (these usually rule it out):

- **Latency-sensitive interactive workloads.** Self-hosted p99 is typically worse than frontier-API p99, not better; the API providers have global edge + capacity that single-tenant hardware can't replicate.
- **Low or sporadic volume.** Capital write-off destroys the unit economics.
- **Capability-sensitive workloads** where frontier-class reasoning is the load-bearing requirement and open-source models trail by 6–18 months.

How to model it when it does fit:

```
Self-hosted cost per case =
  (Hardware capital / useful life in cases)
  + (Electricity per inference × inferences per case)
  + (Ops overhead / cases per month)
  + Tool call cost + HITL cost  ← these don't change

Example (high-volume, latency-tolerant batch workload, ~500k cases/month):
  Hardware: £30k amortised over 3 years × 18M cases = £0.0017/case
  Electricity: ~0.5 kWh per 1k inferences × £0.20/kWh × 5 inferences/case = £0.0005/case
  Ops: £2k/mo / 500k cases = £0.004/case
  Self-hosted subtotal: £0.006/case (vs ~£0.08/case for Sonnet API)

Caveats to surface in the model:
  - p95/p99 latency vs API (often the disqualifier even when £/case wins)
  - Ops staff coverage (24/7? on-call rota? who fixes the model at 03:00?)
  - Capability headroom (does the workload need frontier reasoning?)
  - Capital write-off if workload changes (lock-in risk)
```

**Defended self-hosted economics is graded equally to defended API economics when one of the three "right call" forces above applies.** Without that force, self-hosted is the weaker choice and should not be defaulted to. *"I picked self-hosted because cheaper at scale"* is a weaker answer if you can't name the crossover volume AND defend the latency, AND defend the ops model. *"I picked API by default and modelled self-hosted as the alternative for the data-sovereignty case"* is the stronger framing for most workloads.
