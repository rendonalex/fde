...
- **[CURVEBALL - FDA May 2026 Guidance]** `source_documents`: array of nested objects (SourceDocument entity, see below), required (FDA Requirement 1: "source documents consulted" for 10-year retention + FDA inspection traceability)
- **[CURVEBALL - FDA May 2026 Guidance]** `model_version_adr1`: string, required, immutable, set at extraction time (e.g., "ADR-1 v1.0.2") (FDA Requirement 1: "model identity and version" for traceability if prompt/model changes)
...
- **[CURVEBALL - FDA May 2026 Guidance]** `source_documents` array must be non-empty (at least one source document per case for FDA audit trail)
- **[CURVEBALL - FDA May 2026 Guidance]** `model_version_adr1` must be non-null and match deployed ADR-1 version (e.g., "ADR-1 v1.0")
...
### SourceDocument (nested in AECasePackage)

**[CURVEBALL - FDA May 2026 Guidance]** Added per FDA Requirement 1: "source documents consulted" for 10-year retention + FDA inspection traceability.

**Purpose**: Links each case to originating files/emails/phone transcripts for FDA audit trail.

**Attributes**:
- `filename`: string, required, max 255 characters (original filename, e.g., "hcp_report_2026-05-28.txt")
- `format`: enum [`EMAIL`, `PDF`, `VTT`, `JSON`, `TEXT`], required (source document format)
- `received_at`: ISO 8601 timestamp, required (when intake channel received this source document)
- `sha256_hash`: string, required, 64 characters (SHA-256 hash of source document content for integrity verification in 10-year retention)

**Validation Rules**:
- `filename` must be non-empty
- `sha256_hash` must be valid 64-character hex string (SHA-256 output format)
- At least one `SourceDocument` must exist in `source_documents` array per case (cannot be empty array)
...
8. OUTPUT SCHEMA (Structured JSON)
   {
     ...
     "source_documents": [ { "filename": string, "format": "EMAIL|PDF|VTT|JSON|TEXT", "received_at": "ISO 8601 timestamp", "sha256_hash": string } ],
     "model_version_adr1": "ADR-1 v1.0"
   }

   **[CURVEBALL - FDA May 2026 Guidance]**: Added `source_documents` array (FDA Req 1: source documents consulted) and `model_version_adr1` field (FDA Req 1: model identity and version) to output schema.
...
- **[CURVEBALL - FDA May 2026 Guidance]** `source_documents` array populated with at least one entry (filename, format, received_at, sha256_hash)
- **[CURVEBALL - FDA May 2026 Guidance]** `model_version_adr1` field set to "ADR-1 v1.0" (or current deployed version)
- Processing time: ≤10 min
- Routed to ADR-2 automatically

**Pass Criteria**: Agent processes end-to-end without HITL, all fields match case processor validation (96% accuracy target), source documents and model version fields populated per FDA requirements
...
```json
{
  ...
  "source_documents": [
    {
      "filename": "hcp_report_2026-05-28.txt",
      "format": "TEXT",
      "received_at": "2026-05-28T09:15:00Z",
      "sha256_hash": "a3c5e8f9b2d4c1a7e6f8b3d5c9a2e7f4b1d6c8a9e3f7b2d5c8a4e9f6b3d7c1a8"
    }
  ],
  "model_version_adr1": "ADR-1 v1.0"
}
...
**[CURVEBALL - FDA May 2026 Guidance]**: Added `source_documents` array and `model_version_adr1` field to request schema per FDA Requirement 1 (per-case audit record with model identity/version and source documents consulted).
