...
Score each workstream on the delegation suitability dimensions:
...
Based on suitability scores, assign each workstream to an archetype:
...
**Anti-pattern check**: If a workstream could be solved with static rules, RPA, or a simple script — do not build an agent. Agents are for non-determinism, not for engineering overhead.

### Output
- **Delegation Suitability Matrix**: tasks × dimensions × archetype assignment with rationale and with trade-off analysis.
...
Plot each workstream in the scenario on:
