...
**Token budget** (A22):
- System prompt: ~1,300 tokens (specialty dict + credential dict + location lookup + instructions + 3 few-shot examples)
...
**System prompt token budget**: ~1,300 tokens. Updated only when specialty/credential dictionaries change or few-shot examples are tuned. System prompt changes require re-validation against the 200-record corpus (A10 check).
