...
**Target**: Recover ≥$1.5M in annualized incremental revenue within 6 months of go-live (represents recovering ~50% of the speed-sensitive lost opportunity per A6). *[Rev. 2026-05-13 — CEO Pushback]* This is the ceiling case (A5=30%, win rate ≤75%). The confirmed target is set after the week 2 win-rate check per A5 falsifiable check (see `specs/assumptions.md`): if win rate is 75–90%, M3 revises to $375K–$900K; if win rate is >90%, M3 is replaced with a throughput-scaling metric.
...
What % of inbound requests does MedFlex currently lose to competitors, and for what reason (speed vs. price vs. nurse preference)? This drives the entire revenue-at-stake calculation (A5, A6). If competitive loss is 10% rather than 30%, the financial case shrinks dramatically and the solution priority changes. **Discovery action**: Request 3 months of ServiceNow data showing requests received vs. filled vs. lost. *[Rev. 2026-05-13 — CEO Pushback]* The specific measurement methodology — win rate calculation and speed correlation check — is now defined in `specs/assumptions.md` A5 falsifiable check, executable in the first two weeks of having real ServiceNow data and producing a decision gate for the M3 target.
...
**U10 — What "8 weeks" scope actually means to Marcus** *(Resolved — Rev. 2026-05-13 — CEO Pushback)*
...
*[Rev. 2026-05-13 — CEO Pushback]* **Resolution**: The 6-week board demo scope is now defined. Features 1 (Shift Intake Parser), 2 (AI Candidate Ranker, rule-based), 3 (Coordinator Review Interface), and 4 (Automated Match Submission) are the 6-week deliverable — a system running on real inbound shift requests with end-to-end timestamps and a ranked shortlist for coordinators. Feature 5 (Shift Confirmation Notifier) is deferred to Wave 2. The 8-week engagement continues; the 6-week milestone is the board demo trigger. Full scope documented in `specs/06-client-feedback.md`.
