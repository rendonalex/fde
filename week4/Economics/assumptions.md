...
## A30 — Phase 2 auto-submit HITL rate target: ≤30% after labeled feedback threshold is met

**Assumed value**: Once the Phase 2 ML ranker is deployed and auto-submit is unlocked for high-confidence cases (composite_score ≥ 0.90 AND coordinator edit rate < 10% for that confidence band), the effective HITL rate across all JtD-3 cases will be ≤30% — meaning ≤30% of matches still require coordinator review at BP4, and ≥70% are submitted automatically.

**Reasoning**: The capability spec (`specs/04a-capability-spec-match-selection.md` §2 KPI section) states "Target Phase 2: ≤ 30% HITL as auto-submit confidence threshold is unlocked." This target implies that approximately 70% of the 46,000 annual cases will eventually meet the composite_score ≥ 0.90 threshold for auto-submit. The 30% floor accounts for edge cases, low-confidence pools, hospital-preference-sparse cases (A12), and cases where BP5 rejection re-triggers manual review. At 30% HITL, annual HITL cost drops from $40,526 (MVP) to $12,144/year — a $28,382/year saving — net of the Phase 2 LLM inference cost increase ($690/year, A22).

**Confidence**: Low-Medium. The 30% target is derived from the capability spec design intent, not from measured data. Actual Phase 2 HITL rate depends on: (a) ML ranker accuracy achieved on the A19 corpus, (b) the distribution of composite_score values in production (unknown until MVP runs), and (c) coordinator adoption of auto-submit decisions. The economics model uses 30% as the base Phase 2 scenario; actual rate must be measured and this assumption updated after Phase 2 launch.

**Referenced in**: `specs/token-economics-model.md` (§3.4 HITL Cost, §4.2 Financial Sensitivity Table, §5 Wave 2, §7.4 Phase 2 Economic Unlock Gate)

---

## A31 — Annual platform maintenance cost: 15% of build cost per year

**Assumed value**: Ongoing platform maintenance for JtD-3 (scoring engine updates, integration monitoring, ServiceNow schema compatibility, model version management, coordinator UI patches) costs approximately 15% of the original build cost per year — $9,000/year for the $60,000 JtD-3 build.

**Reasoning**: 15% annual maintenance is the standard software total-cost-of-ownership benchmark for a production AI system with active integrations (ServiceNow read/write, Google Maps API, internal coordinator review UI). It covers: quarterly scoring weight recalibration (A25), ServiceNow API compatibility testing when MedFlex upgrades its instance (A24), Google Maps API key rotation (A26), model version evaluation on Anthropic model releases (A22), and dead-letter queue reconciliation monitoring. This does not include Wave 2 ML ranker development cost, which is treated as a separate capital investment in the Wave 2 roadmap.

**Confidence**: Medium. 15% is a standard industry benchmark for production software maintenance. Actual cost depends on: how frequently ServiceNow schema changes (A24 risk), whether scoring weight recalibration requires FDE involvement or can be done by MedFlex IT, and whether the coordinator UI requires ongoing UX iteration. Maintenance cost is likely to be lower in years 2–3 once the integration layer stabilizes.

**Referenced in**: `specs/token-economics-model.md` (§4.1 Standard Business Case Model, §4.2 Financial Sensitivity Table, §7.2 Quarterly Reviews)

---

## A32 — JtD-1 Phase 2 parser HITL rate target: ≤10% after Wave 2 prompt re-tuning

**Assumed value**: After Wave 2 system prompt improvement using the accumulated HITL correction corpus (~750–1,100 labeled `HUMAN_CORRECTED` examples at 15% HITL rate × 184/day × 20 production working days), the JtD-1 parser HITL rate drops from the MVP ≤15% target (A10) to ≤10%.

**Reasoning**: Wave 2 incorporates real HITL correction cases as updated few-shot examples in the system prompt, targeting the highest-frequency failure modes observed in production (typically: ambiguous location references, unknown specialty abbreviations, and relative date expressions without explicit year). The improvement from 15% → 10% HITL represents a ~33% reduction in exception routing achievable through prompt engineering alone — not ML fine-tuning. At 184/day × 20 production days = 3,680 auto-parses and ~552 HITL cases per month, the corpus reaches the useful threshold for few-shot updates within 4–6 weeks of live operation. Economic impact: annual HITL cost drops from $21,298/year to $14,196/year — a $7,100/year improvement for ~1–2 FDE engineer-weeks of prompt engineering work.

**Confidence**: Medium. The 15% → 10% improvement is directionally expected from few-shot corpus updates; actual rate depends on whether production HITL failures are concentrated in addressable prompt engineering gaps (e.g., new abbreviations, format variations) or in genuinely ambiguous requests (e.g., multi-nurse submissions, missing hospital location data). If ambiguous requests dominate, the structured intake template (T2, deferred to Wave 2 for high-volume hospital partners) may be required to reach 10% rather than prompt tuning alone.

**Referenced in**: `specs/token-economics-model-intake-parser.md` (§3.5 Total Agent Cost, §4.2 Financial Sensitivity Table, §5.4 Wave 2 Self-Financing, §7.3 Wave 2 Trigger Conditions, §8.2 MT-1.6 Delegation Rationale)
