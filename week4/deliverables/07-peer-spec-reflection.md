# Reflection: Build Attempt vs. Peer Review — Spec 04a-capability-spec-intake-Priya-Gupta

## What the build attempt revealed

Claude-code 8 clarification questions surfaced **implementation-level blockers**: language/framework, persistence layer type, coordinator queue mechanism, credential CSV schema, fuzzy-match algorithm, LOCATION_INCOMPLETE detection logic, POSSIBLE_MISSED_CREDENTIAL heuristics, and which fields count as "required" for the −0.25 null deduction. These are the questions the builder hits *the moment they try to write the first line of code* — not theoretical concerns, but hard stops.
