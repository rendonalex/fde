...
9. [Model Hosting Decision: Frontier API vs. Self-Hosted](#38-model-hosting-decision-frontier-api-vs-self-hosted)
...
1. **Parsing labor is pure overhead.** Current coordinator parsing costs $141,900/year (A1, A4, A7, A16) — a data-entry burden with no judgment value that an LLM eliminates for 85% of cases at $0.003/case (A10, A22). Every fill-time delay begins here.

2. **HITL rate is the only economic lever that matters.** Token cost ($0.003/case) and tool calls ($0.001/case) together are <1% of agent spend. Human review of the 15% exception path ($0.463/case) accounts for >99% of all agent-side cost. Cost optimization means HITL rate reduction, not LLM tuning.
...
|---|---|---|---|
| Conservative (+50% token cost, 20% HITL) | $113,100 | 3.2 months | 680% |
| **Base case ($0.003/case, 15% HITL)** | **$120,200** | **3.0 months** | **729%** |
| Optimistic (−30% token cost, 10% HITL, A32) | $127,500 | 2.8 months | 779% |
...
JtD-1 is an LLM-native workstream: every inbound request triggers one Claude Haiku 4.5 API call. Token costs are explicit, fully attributable, and fixed per case.
...
| **Total per case** | **~1,500 in / ~400 out** | **$0.0028/case** | A22 |

Calculation: (1,500 × $0.80 + 400 × $4.00) / 1,000,000 = $2.80 / 1,000,000 = $0.0028/case (A22).

The system prompt (~1,300 tokens) is stable across sessions. Prompt caching would reduce repeat input costs further, but is not modeled here because $0.0028/case is already at 0.09% of the $3.08/case baseline — further optimization has negligible economic impact.
...
**Cost decomposition**: Token + tool costs ($0.0038/case) = <1% of total agent cost. HITL = >99%. The entire economic optimization roadmap for JtD-1 is HITL rate reduction, not token engineering.
...
After analyzing 5 model outputs for the MedFlex intake parser, Claude Haiku 4.5 (Model C) is the production model: it scores 8.78/10 accuracy — higher than Sonnet 4.6 (8.25/10) and within 1.7% of the best performers (GPT-5/Opus at 8.93/10), while costing ~73% less per case. The biggest errors in the test set were date disambiguation failures — Gemini 2.5 hallucinated a future year (2028) to reconcile a Saturday/date conflict; Sonnet 4.6 prioritized day-of-week over the explicit date. Since HITL cost dominates at >99% of agent spend, the economic impact of the model choice is small ($354/year token difference vs. Sonnet), but Haiku's higher accuracy also reduces downstream HITL risk.   
...
### 3.8 Model Hosting Decision: Frontier API vs. Self-Hosted

**Default: Frontier API.** Claude, GPT-4o, and Gemini are the correct baseline for this workload. They offer state-of-the-art capability, sub-second inference latency, no capital expenditure, and no ops overhead. Any deviation from the API default requires an explicit forcing condition.

**Three conditions that justify self-hosted:**

| Forcing Condition | Applies to JtD-1? |
|---|---|
| Data-sovereignty / compliance rules out external API providers (regulated verticals, classified workloads, data-residency law) | No — MedFlex shift request data does not carry a compliance constraint that prohibits Anthropic API usage (A22). If a future client engagement introduces data-residency law or classified workload requirements, this assessment must be revisited. |
| Volume genuinely amortises capital at the crossover point (typically millions of cases/year for a Haiku-equivalent workload) | No — JtD-1 runs at 46,000 cases/year. Self-hosted infrastructure (GPU hardware, cooling, ops) carries ~$150K–$300K/year all-in cost for a comparable open-source model on owned hardware. Crossover with Anthropic API pricing ($129/year in token cost at current volume) does not occur until approximately 10–20× current volume (~500K–1M cases/year). MedFlex is not near this threshold. |
| Fine-tuning need produces a model the client owns and re-trains on their own data | No — JtD-1 requires extraction accuracy, not a fine-tuned model. Accuracy improvement is achieved through prompt engineering (few-shot examples, dictionary updates) and HITL corpus feedback, not model ownership. |

**Cost shape comparison (for reference if a future engagement triggers one of the above):**

| Cost Component | Frontier API Model | Self-Hosted Open-Source Model |
|---|---|---|
| Capital | Negligible | $150K–$300K+ GPU hardware amortised over 3–5 years |
| Electricity + cooling | None | $15K–$40K/year depending on hardware and location |
| Ops overhead | None | 0.25–0.5 FTE to maintain model health, updates, uptime |
| Marginal per-inference cost | $0.0028/case (token rate × volume) | Near-zero after capital recovery |
| Inference latency | Frontier-fast (Anthropic-managed infrastructure) | Typically higher than frontier APIs; depends on hardware tier |
| **Total at 46,000 cases/year** | **~$21,700/year (token + HITL + infra)** | **~$190K–$350K+/year (capital + ops + HITL)** |

At JtD-1's current volume, self-hosted is approximately 9–16× more expensive than the API path when all costs are counted correctly. "Self-hosted because cheaper at scale" requires naming the crossover volume, the latency tolerance of the workload, and how ops gets handled — none of which favour self-hosted at 46,000 cases/year.

**Conclusion:** API by default is the correct and defensible choice for JtD-1. Revisit only if MedFlex volume grows to ~500K+ cases/year, a data-residency constraint is introduced, or a client ownership requirement for the underlying model arises.
...
| Annual agent operating cost | $21,700/year | §3.6 |
| **Annual saving** | **$120,200/year** | §3.6 |
| Build cost | $30,000 (2 weeks × $15K/week) | A21 |
| Annual maintenance | $4,500/year (15% × $30K) | A31 |
| **Payback period** | **3.0 months** ($30K / ($120,200/12)) | |
| Year 1 net (saving − build − maintenance) | $85,700 | |
| Year 1 ROI | 248% | |
| 3-year total saving | $360,600 | |
| 3-year total investment | $43,500 ($30K + 3 × $4.5K) | |
| **3-year net value** | **$317,100** | |
| **3-year ROI** | **729%** | |
...
|---|---|---|---|---|---|
| Conservative | $0.0042 (+50%) | 20% | $113,100 | 3.2 months | 680% |
| **Base case** | **$0.0028** | **15% (A10)** | **$120,200** | **3.0 months** | **729%** |
| Optimistic | $0.002 (−30%) | 10% (A32) | $127,500 | 2.8 months | 779% |

**Conservative scenario calculation**: HITL cost = 0.20 × (7/60) × $26.44 = $0.617/case. Total agent cost = $0.625/case. Annual saving = $141,900 − $28,750 = $113,150/year.

**Robustness**: The business case is insensitive to token price movement — a 50% price increase shifts payback by only 0.2 months. The range of 3-year ROI (680%–779%) is narrow, confirming that JtD-1 economics are not at risk from LLM pricing volatility. The only scenario that materially degrades ROI is a sustained HITL rate above 25% — which would only occur if the A10 accuracy target (≥85%) is not met at launch.
...
|---|---|---|---|---|
| JtD-1 — Shift Intake Parser | $30K | $120,200 | 3.0 months | Weeks 1–2 |
...
Year 1 end:  JtD-1 generates $85,700 net (after build + maintenance)
```
...
- Measure actual token consumption per case — compare against the $0.0028/case budget (A22); flag if average exceeds $0.004
...
| Cost per parse | ≤$0.003/case (A22) | Budget exceeded | Trim system prompt tokens; evaluate next-generation Haiku for pre-filtering (Wave 3) |
...
|---|---|---|
| Cost per parse | ≤$0.003/case | Claude API usage logs (`usage.input_tokens`, `usage.output_tokens`) |
...
- **Model release evaluation (A22, A33)**: On each Anthropic model release, run the 200-record validation corpus against the new model. Compare accuracy, HITL rate, tokens per case, and latency. If the new model achieves equivalent accuracy at lower cost, update model ID in config — it is a parameterized field in the integration contract (A22). **Claude Haiku 4.5 ($0.80/$4.00 per 1M tokens) is the production model as of 2026-05-20 (A33)** — validated at 8.78/10 composite accuracy, exceeding Sonnet 4.6 (8.25/10) at 27% of the token cost. Evaluate future releases against this baseline.
...
- **Hosting model reassessment**: If annual case volume grows to ≥500K cases/year, re-evaluate the API vs. self-hosted crossover using the cost shape framework in §3.8. Below that threshold, frontier API remains the correct default.
...
| MT-1.2 | LLM extraction call | Fully Agentic | $0.0028/case | DELEGATE |
...
Delegation justification: this is the primary economic trade. The LLM translates free-text hospital requests into a strict validated schema at $0.0028/case — replacing $3.08/case of coordinator labor for the 85% auto-path. Instruction specificity (A22 system prompt: role, output schema, domain dictionaries, per-field confidence rules, 3 few-shot examples) is sufficient to produce consistent, validatable JSON output.
...
Economic impact: $0.0028/case in token cost replaces $3.08/case in coordinator labor for 85% of cases. The remaining $0.463/case in HITL cost handles the 15% exception path.
...
- Total: $0.315/case (89.8% reduction from $3.08/case baseline)
